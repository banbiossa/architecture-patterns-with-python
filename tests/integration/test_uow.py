...
def get_allocated_batch_ref(session, orderid, sku):
    ...
    [[batchref]] = session.execute(
        "SELECT b.reference FROM allocations JOIN batches AS b ON b.id = batch_id"
...
def test_uow_can_retrieve_a_batch_and_allocate_to_it(session_factory):
    ...
    insert_batch(session, "b1", "COMPLICATED-LAMP", 100, None)
    session.commit()
    ...
    batchref = get_allocated_batch_ref(session, "o1", "COMPLICATED-LAMP")
    assert batchref == "b1"
...
def test_rolls_back_uncommitted_work_by_default(session_factory):
    uow = unit_of_work.SqlAlchemyUnitOfWork(session_factory)
    with uow:
        insert_batch(uow.session, "b1", "AREALLYDIFFERENTSKU", 100, None)
...
def test_concurrent_updates_to_version_are_not_allowed(postgres_session_factory):
    ...
    insert_batch(session, batch, sku, 100, None)
    session.commit()
